...
export function isDefaultRetentionPolicy(policy) {
...
export function retentionCandidateKey(item) {
 ...
 */
export function retentionCandidateTimeMs(item) {
    ...
    if (typeof item.mtimeMs === 'number' && Number.isFinite(item.mtimeMs))
        return item.mtimeMs;
    return Number.NaN;
}
 ...
 */
function keepByPeriod(decorated, keep, limit, periodOf) {
    if (limit <= 0)
        return;
    ...
    for (const item of decorated) {
        ...
        if (covered.has(period))
            continue; // 该周期已由更新项代表
        if (covered.size >= limit)
            continue; // 已覆盖 limit 个周期（更早周期不再保护）
        covered.add(period);
        if (!keep.has(item.key))
            keep.add(item.key); // 该周期最新项入选
    }
}
/**
 ...
 */
export function selectRetentionKeepers(items, policy, now = new Date()) {
    void now;
    const keep = new Set();
    if (items.length === 0)
        return keep;
    ...
    if (policy.keepLast > 0) {
        for (const item of decorated.slice(0, policy.keepLast))
            keep.add(item.key);
    }
 ...
 */
export function validateRetentionPolicy(raw) {
...
export function hasRetentionTiers(policy) {
 ...
 * - 不修改入参。
 */
export function selectPruneCandidatesByPolicy(metas, policy = DEFAULT_RETENTION_POLICY, selectLegacy) {
    if (isDefaultRetentionPolicy(policy)) {
        // 快速路径：由宿主传入 core 的 selectPruneCandidates 保持逐字等价；未传则走下面通用路径
        if (selectLegacy !== undefined)
            return selectLegacy(metas, policy.keepLast);
    }
    if (metas.length === 0)
        return [];
    const keepers = selectRetentionKeepers(metas.map((m) => ({ id: m.id, createdAt: m.createdAt })), policy);
...
export function retentionPolicySummary(policy) {
    return {
