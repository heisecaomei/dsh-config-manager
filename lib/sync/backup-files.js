...
import { isDefaultRetentionPolicy, selectRetentionKeepers } from './retention-policy.js';
 ...
 */
export async function pruneAutoBackupsByPolicy(exportsDir, policy) {
    ...
    if (auto.length === 0)
        return [];
    const keepers = selectRetentionKeepers(auto.map((m) => ({ name: m.name, mtimeMs: m.mtimeMs })), policy);
    const removed = [];
    for (const meta of auto) {
        if (keepers.has(meta.name))
            continue;
        try {
            await fs.rm(meta.path, { force: false });
            removed.push(meta.name);
