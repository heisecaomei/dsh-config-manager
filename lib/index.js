...
import { DEFAULT_RETENTION_POLICY, selectPruneCandidatesByPolicy } from './sync/retention-policy.js';
import { selectPruneCandidates } from './core/backup.js';
...
function makeRoutes(deps) {
     ...
     */
    const retentionPruneSelector = (metas, policy) => selectPruneCandidatesByPolicy(metas, policy, selectPruneCandidates);
    /**
        ...
        // mutation 动作经 withMutationGate（GLOBAL 锁 + SAFE MODE 闸门）；status 只读不加锁。
        {
            ...
            handler: async (req, res) => {
                if (!LIFECYCLE_ENABLED) {
                    writeJson(res, 503, { ok: false, code: 'feature-disabled', message: 'disaster recovery is temporarily disabled' });
                    return;
                }
                if (!isLoopbackRequest(req)) {
                    writeJson(res, 403, { error: 'forbidden: loopback-only' });
                    ...
                    await withMutationGate('lifecycle-snapshot', async (rq, rs) => {
                        ...
                        try {
                            ...
                            const meta = await lifecycle.snapshot({
                                ...
                                ...(Array.isArray(body?.['tags']) ? { tags: body['tags'].map((t) => String(t)) } : {}),
                            });
                            writeJson(rs, 200, { ok: true, id: meta.id, kind: meta.kind, totalBytes: meta.totalBytes });
                        }
                        catch (error) {
                            if (error instanceof EnvironmentLockUnavailableError) {
                                writeJson(rs, 423, { error: error.message, code: 'mutation-locked' });
                                return;
                            }
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (action === 'off') {
                        ...
                        if (!r.ok) {
                            writeJson(res, 409, { ok: false, code: r.code, message: r.message });
                            return;
                        }
