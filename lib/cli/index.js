...
import { collectBackupEntries, buildSectionFlags, parseSectionsArg, DEFAULT_BACKUP_SECTIONS, OFFLINE_UNAVAILABLE_SECTIONS, } from '../core/backup-plan.js';
...
function parseCliDataDir(argv) {
    const options = { command: 'recover-stale-lock', dryRun: false, profile: 'web', yes: false, list: false, wipeConfig: false, json: false, positionals: [] };
...
function resolveCliVersion() {
    try {
        ...
        const parsed = JSON.parse(fssync.readFileSync(pkgUrl, 'utf8'));
...
export function resolveExportsDir(flag, env = process.env) {
    if (flag !== undefined && flag !== '')
        return flag;
    return path.join(resolveDshHome(env), 'dsh-config-manager', 'exports');
}
/** 定位待校验目标：显式目标（文件名或路径）优先，否则列出导出目录下全部 *.zip（名称升序） */
async function resolveVerifyTargets(dataDir, explicit) {
...
function printVerifyResult(r, io) {
...
async function runVerify(options, io, env) {
    ...
    if (!resolved.ok) {
        ...
        if (options.json)
            io.log(stringifyJsonSafe({ error: resolved.error }, { space: 2 }));
        else
    ...
    for (const target of resolved.targets)
        results.push(await verifyBackupZip(target));
    if (options.json) {
    ...
    if (failed.length > 0) {
        if (!options.json)
            io.error(`${failed.length} 个备份未通过自检 / ${failed.length} backup(s) failed verification`);
        return 1;
    }
    return 0;
}
...
function printBackupPlan(col, outPath, io) {
...
async function runBackup(options, io, env) {
    ...
    if (options.out !== undefined && options.out !== '') {
        ...
        if (await pathExists(outPath)) {
            ...
            for (let i = 2; await pathExists(candidate); i += 1)
                candidate = `${base}-${i}.zip`;
            outPath = candidate;
        }
    }
    else {
    ...
    for (const w of collection.warnings)
        io.error(`  - ${w}`);
...
async function randomHex4() {
    const { randomBytes } = await import('node:crypto');
    return randomBytes(4).toString('hex');
}
/** 导出目录内取不冲突的文件名（复用既有去重语义：foo.zip → foo-1.zip） */
async function resolveNonCollidingName(dir, desired) {
    let existing = [];
    try {
