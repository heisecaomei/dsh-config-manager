...
import { verifyChecksums } from '../utils/hashing.js';
import { CHECKSUMS_FILE, MANIFEST_FILE, parseManifest } from '../schema/manifest.js';
import { describeVersion, isSupported } from '../schema/versions.js';
import { SECTION_FILE_PREFIXES, SECTION_JSON_PATHS } from '../schema/config.js';
import { isArchiveBlob } from '../security/encryption.js';
import { parseChecksumsTable } from '../security/integrity.js';
import { createHardenedZipParser } from '../security/zip-security.js';
/** 未纳入 checksums 表参与的条目（这两个是校验表自身的容器，按导出器语义不参与） */
const NON_DATA_ENTRIES = new Set([MANIFEST_FILE, CHECKSUMS_FILE]);
/**
 ...
 */
class LazyEntryMap extends Map {
    archive;
    unreadable;
    onUnreadable;
    constructor(archive, unreadable, onUnreadable) {
...
function makeResult(file, verdict, errors, warnings) {
 ...
 */
export async function verifyBackupZip(zipPath) {
    const errors = [];
    const warnings = [];
    /* ---------------- 1. 存在 / 可读 ---------------- */
    let sizeBytes;
...
//# sourceMappingURL=backup-verify.js.map