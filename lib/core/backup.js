 ...
 */
export function resolveFileTargetRel(adapter, ref, profile) {
    if (adapter === 'plugins') {
        const profileDir = `profiles/${profile !== undefined && profile !== '' ? profile : 'web'}`;
        if (ref === 'pnpm-workspace.yaml')
            return normalizePath(`${profileDir}/pnpm-workspace.yaml`);
...
export function defaultRetentionPolicyLike() {
 ...
 */
export const fallbackPruneSelector = (metas, policy) => selectPruneCandidates(metas, policy.keepLast);
