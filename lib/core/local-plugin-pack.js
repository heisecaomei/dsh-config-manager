...
function startsWithIgnoreCase(spec, prefix) {
...
export function classifyPluginSpec(spec) {
    ...
    if (s === '')
        return 'registry';
    if (startsWithIgnoreCase(s, LINK_PREFIX))
        return 'link';
    if (startsWithIgnoreCase(s, FILE_PREFIX))
        return 'file';
    for (const p of GIT_PREFIXES) {
        if (startsWithIgnoreCase(s, p))
            return 'git';
    }
    if (startsWithIgnoreCase(s, 'workspace:'))
        return 'registry';
    return 'registry';
}
/** 是否本地源（换机后必然不可达，需打包） */
export function isLocalPluginSpec(spec) {
 ...
 */
function hasSchemePrefix(spec) {
    if (/^[a-zA-Z]:[\\/]/.test(spec))
        return false; // 盘符路径，不是 scheme
 ...
 */
export function safePackageFileFragment(pkgName) {
...
export function tarballNameFor(pkgName, version) {
 ...
 */
export function resolveLocalPluginPath(spec, opts) {
    let raw = spec.trim();
    if (startsWithIgnoreCase(raw, LINK_PREFIX))
        raw = raw.slice(LINK_PREFIX.length);
    else if (startsWithIgnoreCase(raw, FILE_PREFIX))
        raw = raw.slice(FILE_PREFIX.length);
    raw = raw.trim();
    // `~` 展开（`~/x` 或 `~`）
    if (raw === '~')
        return opts.homeDir;
 ...
 */
export function rewriteLocalSpec(spec, opts) {
    void spec;
 ...
 * 导入端据此避免二次重写。
 */
export function isPackedLocalSpec(spec) {
    if (typeof spec !== 'string')
        return false;
    const s = spec.trim();
    if (!startsWithIgnoreCase(s, FILE_PREFIX))
 ...
 */
export function parsePackOutput(stdout, expectedPkg, version) {
    ...
    if (last !== undefined && last.endsWith('.tgz'))
        return last;
 ...
 */
export async function packLocalPlugins(opts) {
    const packed = [];
    const rewritten = {};
    const warnings = [];
    ...
    for (const plugin of candidates) {
        ...
        try {
            ...
            if (result.code !== 0) {
                warnings.push(`本地插件 ${plugin.name} 打包失败（npm pack 退出码 ${result.code ?? 'null'}）：${firstLine(result.stderr) || firstLine(result.stdout) || '无输出'}`);
                continue;
            }
            ...
            if (data.byteLength > maxBytes) {
                warnings.push(`本地插件 ${plugin.name} 的 tarball 超过上限（${data.byteLength} > ${maxBytes} 字节），已跳过打包`);
                continue;
            }
 ...
 */
export function isBareLocalPath(spec) {
    if (typeof spec !== 'string')
        return false;
    const s = spec.trim();
    if (s === '')
...
//# sourceMappingURL=local-plugin-pack.js.map