...
import { stateFromExports, statesEqual } from './config-state.js';
import { listConfigSnapshots, pruneConfigSnapshots, readConfigSnapshotMeta, restoreConfigSnapshot, saveConfigSnapshot, toUndoCandidate, } from './config-snapshot.js';
import { canRedo, canUndo, planRedo, planUndo, steppedIds } from './undo.js';
import { DebouncedWatcher, EchoRegistry } from './watcher.js';
import { updateConfigSnapshotMeta } from './config-snapshot.js';
/** 启动关键配置文件（相对 homeDir）——自动快照最需要覆盖的那一批 */
export const BOOT_CRITICAL_RELS = [
    'settings.yaml',
    'settings.json',
    'cordis.patch.yml',
    '.env',
...
export function profileCriticalRels(profile) {
    return [
...
/** 需要监听变更的目录（绝对路径；去重） */
export function watchDirsFor(homeDir, profile) {
    const dirs = [
        homeDir,
...
export function echoCandidatePaths(homeDir, profile) {
...
export class ConfigLifecycle {
     ...
     */
    async captureSections(only) {
        const sections = new Map();
        const failed = [];
        for (const adapter of this.opts.adapters) {
            if (only !== undefined && !only.includes(adapter.id))
                continue;
            try {
     ...
     */
    async saveSnapshot(req, captured) {
    ...
    async undo() {
        ...
        try {
            ...
            pre = await this.saveSnapshot({ kind: 'pre-restore', reason: `before-undo:${plan.targetId}`, trigger: 'undo' }, captured);
    ...
    async replay(id) {
        ...
        const run = async () => restoreConfigSnapshot({
            dir: this.opts.dir,
            id,
     ...
     * 未登记、或读不到（新增/删除）→ 真实变更。
     */
    async isEchoBatch(events) {
        if (this.echo.size === 0 || events.length === 0)
            return false;
        let checked = false;
        for (const event of events) {
            const abs = normalizePath(path.resolve(event.dir, event.filename));
            if (!this.echo.has(abs))
                return false;
            let data;
            try {
            ...
            if (!this.echo.isEcho(abs, sha256Hex(data)))
                return false;
            checked = true;
        }
        return checked;
    ...
    startAutoSnapshot() {
        if (this.disposed)
            return;
        if (this.opts.autoEnabled === false)
            return;
        if (this.opts.watchFactory === undefined)
            return;
    ...
    reconcileWatchSet() {
        ...
        if (desired.length === current.length && desired.every((d, i) => d === current[i]))
            return;
    ...
    /** 处理一批事件：回声判定 → 采集 → 落盘。 */
    async flushOnce(events) {
        if (await this.isEchoBatch(events))
            return; // 恢复自写的回声 → 不产生快照
        const captured = await this.captureSections();
        // 第二层回声兜底（见 lastReplayState）：恢复动作刚把配置写成 X，此刻采集到的仍是 X
        // → 这批事件是窗口之后才投递的恢复自写回声，不得产生快照（否则挡住重做）。
        if (this.lastReplayState !== null && statesEqual(captured.state, this.lastReplayState))
            return;
...
//# sourceMappingURL=config-lifecycle.js.map