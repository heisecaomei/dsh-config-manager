...
export const CLIENT_BUNDLE_RUNTIME_WHITELIST = [
    'react',
    'react-dom',
    'react-dom/client',
    'react/jsx-runtime',
 ...
 * 漏掉——而那种内容必然是真正的注释。
 */
export const BUNDLE_SCAN_PASSES = [
 ...
 * 多趟并集负责兜底。
 */
export function stripJsComments(source, templateMode = true, quotesOpaque = false) {
    let out = '';
    let i = 0;
...
function literalSpecifier(argText) {
 ...
 */
export function extractRequireHits(strippedCode) {
    const hits = [];
...
/** 按给定配置跑一趟扫描。 */
export function scanBundleRequiresPass(source, config = 'template', whitelist = CLIENT_BUNDLE_RUNTIME_WHITELIST) {
 ...
 * 不可能造成「全部趟一起漏报」。
 */
export function scanBundleRequires(source, whitelist = CLIENT_BUNDLE_RUNTIME_WHITELIST, passConfigs = BUNDLE_SCAN_PASSES) {
...
//# sourceMappingURL=bundle-scan.js.map