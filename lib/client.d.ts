...
//#endregion
//#region src/sync/retention-policy.d.ts
/**
...
interface RecoveryLockRecoverResult {
  ok: boolean;
  removed: boolean;
  state: string;
}
...
//#endregion
//#region src/client/lifecycle/lifecycle-api.d.ts
/** 配置快照种类（与 core/undo.ts 的 ConfigSnapshotKind 同构）。 */
type LifecycleSnapshotKind = 'manual' | 'auto' | 'undo' | 'pre-restore' | 'baseline';
/** 单个配置状态快照的元信息（列表展示用；不含分区数据）。 */
interface LifecycleSnapshotMeta {
  id: string;
  createdAt: string;
...
type CrashKind = 'session-corrupt' | 'bundle-check' | 'patch-tree' | 'unknown';
/** 建议动作（与 core/crash-report.ts 的 CrashAdvice 同构）。 */
type CrashAdvice = 'none' | 'restore-last-good' | 'repair-session' | 'check-bundles' | 'check-patch-tree';
/** GET /crash 响应。 */
...
declare class LifecycleApi {
   ...
   */
  undo(): Promise<LifecycleOutcome>;
  /** POST /lifecycle/redo：重做上一次撤销（仅当撤销后没有新变更时可成功）。 */
  redo(): Promise<LifecycleOutcome>;
  ...
  crash(): Promise<CrashReport>;
  /** GET /rescue：救援模式状态。 */
  rescueStatus(): Promise<RescueStatus>;
  /**
   * POST /rescue {action:'on', confirm:true}：进入救援模式。
   * confirm 必须由调用方在显式确认弹窗后传入 —— 本层绝不代填。
   */
  rescueOn(): Promise<RescueActionResult>;
  /** POST /rescue {action:'off'}：退出救援模式（从备份还原）。 */
  rescueOff(): Promise<RescueActionResult>;
}
//#endregion
...
declare const zh$3: {
  ...
  readonly 'toast.copied': '已复制到剪贴板';
  readonly 'toast.copyFailed': '复制失败，请手动选择文本复制';
  readonly 'toast.refreshed': '已刷新';
  readonly 'overview.subtitle': '配置健康与快速操作一览（数据来自各功能页的只读状态）';
  readonly 'overview.refresh': '刷新';
  readonly 'overview.loading': '正在加载总览…';
  readonly 'overview.health.ok': '配置状态良好';
  readonly 'overview.health.noBackup': '尚无任何备份 —— 建议先创建一个备份或快照';
  readonly 'overview.health.scheduleFailed': '上次定时备份失败，建议检查备份设置';
  readonly 'overview.health.recovery': '存在未解决的恢复事项，需要处理';
  readonly 'overview.health.recoveryAction': '前往「事故恢复」处理';
  readonly 'overview.metric.backups': '备份文件';
  readonly 'overview.metric.snapshots': '安全快照';
  readonly 'overview.metric.schedule': '定时备份';
  readonly 'overview.metric.sync': '远程同步';
  readonly 'overview.state.on': '开启';
  readonly 'overview.state.off': '关闭';
  readonly 'overview.meta.lastBackup': '最近：{time}';
  readonly 'overview.meta.noBackup': '尚无记录';
  readonly 'overview.meta.scheduleOn': '已开启 · 上次 {time}';
  readonly 'overview.meta.scheduleOff': '未开启（建议开启自动保护）';
  readonly 'overview.meta.scheduleFail': '上次备份失败';
  readonly 'overview.meta.syncOn': '已配置 · 上次 {time}';
  readonly 'overview.meta.syncOff': '未配置';
  readonly 'overview.meta.never': '从未运行';
  readonly 'overview.time.now': '刚刚';
  readonly 'overview.time.min': '{n} 分钟前';
  readonly 'overview.time.hour': '{n} 小时前';
  readonly 'overview.time.day': '{n} 天前';
  readonly 'overview.quick.title': '快速操作';
  readonly 'overview.quick.backup': '立即备份';
  readonly 'overview.quick.backupHint': '全量快照（不含密钥），随时可回滚';
  readonly 'overview.quick.backupDone': '备份完成';
  readonly 'overview.quick.backupFailed': '备份失败';
  readonly 'overview.quick.export': '导出备份';
  readonly 'overview.quick.exportHint': '生成可迁移的 ZIP 备份文件';
  readonly 'overview.quick.import': '导入恢复';
  readonly 'overview.quick.importHint': '从备份文件恢复或迁移到本机';
  readonly 'overview.quick.sync': '远程同步';
  readonly 'overview.quick.syncHint': '跨设备保持配置一致（Git / WebDAV）';
  readonly 'overview.activity.title': '最近活动';
  readonly 'overview.activity.empty': '暂无操作记录（执行备份 / 导入 / 同步后在这里显示）';
  readonly 'overview.suggest.title': '建议';
  readonly 'overview.suggest.schedule': '开启定时备份，让 DSH 自动保护你的配置';
  readonly 'overview.suggest.sync': '配置远程同步，跨设备恢复更省心';
  readonly 'overview.empty.title': '开始保护你的配置';
  readonly 'overview.empty.body': '创建第一个备份或快照后，这里会展示配置健康状态与最近活动。';
  readonly 'overview.running': '{count} 个任务进行中';
  readonly 'overview.kind.import': '导入';
  readonly 'overview.kind.restore': '恢复';
  readonly 'overview.kind.rollback': '回滚';
  readonly 'overview.kind.profile-switch': '切换配置档案';
  readonly 'overview.kind.profile-delete': '删除配置档案';
  readonly 'overview.kind.profile-rename': '重命名配置档案';
  readonly 'overview.kind.profile-save': '保存配置档案';
  readonly 'overview.kind.profile-import': '导入配置档案';
  readonly 'overview.kind.sync-apply': '同步应用';
  readonly 'overview.kind.autosync': '自动同步';
  readonly 'overview.kind.recovery': '恢复事项';
  readonly 'overview.kind.backup': '备份';
  readonly 'overview.kind.snapshot-delete': '删除快照';
  readonly 'overview.kind.snapshot-prune': '清理快照';
  readonly 'overview.kind.other': '其他操作';
  ...
  readonly 'import.stage.select': '选择';
  readonly 'import.stage.analyze': '分析';
  readonly 'import.stage.decide': '预览与决策';
  readonly 'import.stage.confirm': '确认';
  readonly 'import.stage.execute': '执行';
  readonly 'import.stage.done': '完成';
  readonly 'import.stepper.label': '第 {current} 步，共 {total} 步：{label}';
  ...
  readonly 'snapshots.subTab.restore': '安全快照';
  readonly 'snapshots.subTab.files': '备份文件';
  readonly 'snapshots.subTab.schedule': '定时备份';
  readonly 'snapshots.subTab.recovery': '事故恢复';
  readonly 'snapshots.empty.title': '尚无安全快照';
  readonly 'snapshots.empty.body': '安全快照在导入 / 恢复前自动创建，用于一键回滚；也可立即手动创建一个。';
  readonly 'snapshots.empty.viewFiles': '查看备份文件';
  readonly 'snapshots.empty.runBackup': '立即备份';
  readonly 'backupFiles.noteUnreadable': '（备注不可读）';
  ...
  readonly 'snapshots.deleted': '已删除快照';
  ...
  readonly 'retention.title': '保留策略';
  readonly 'retention.hint': '控制自动清理保留多少份（快照与定时备份共用）。置顶的快照与未收敛恢复点恒豁免自动清理。';
  readonly 'retention.keepLast': '最近保留';
  readonly 'retention.keepLastHint': '按时间保留最近 N 份；0 = 不按「最近」保留';
  readonly 'retention.keepMonthly': '每月保留';
  readonly 'retention.keepMonthlyHint': '每月额外保留最新 1 份，最多覆盖 N 个月；0 = 关闭';
  readonly 'retention.keepYearly': '每年保留';
  readonly 'retention.keepYearlyHint': '每年额外保留最新 1 份，最多覆盖 N 年；0 = 关闭';
  readonly 'retention.unit': '份';
  readonly 'retention.months': '个月';
  readonly 'retention.years': '年';
  readonly 'retention.tiersOff': '未启用分层（等价于「只保留最近 N 份」）';
  readonly 'retention.summary': '最近 {keepLast} 份 · 每月 {keepMonthly} · 每年 {keepYearly}';
  readonly 'retention.invalid': '保留策略取值非法，请检查各字段（整数且在允许范围内）';
  readonly 'retention.appliesTo': '生效于：导入前快照 + 定时备份产物';
  readonly 'backupFiles.title': '备份文件';
  readonly 'backupFiles.name': '文件名';
  readonly 'backupFiles.source': '来源';
  readonly 'backupFiles.size': '大小';
  readonly 'backupFiles.time': '修改时间';
  ...
  readonly 'backupFiles.deleted': '已删除备份文件「{name}」';
  ...
  readonly 'about.diag.label': '插件清单来源';
  readonly 'about.diag.profileDir': '目录：{path}';
  readonly 'about.diag.profile': 'profile：{profile}';
  readonly 'about.diag.pluginCount': '已识别插件：{count} 个';
  readonly 'about.diag.manifestUnreadable': '该目录下读不到 package.json —— 插件清单必然为空；请确认 DSH 的 profile 与 DSH_HOME 是否与此处一致。';
  readonly 'about.diag.bundles': 'bundles：{bundles}';
  readonly 'about.diag.hint': '若与实际不符，请据此检查 DSH 启动参数 --profile 与 DSH_HOME 环境变量。';
  ...
  readonly 'profiles.subtitle': '把当前 DSH 配置保存为可切换的档案（Work / Personal / …），随时切换；切换含预览 + 自动快照 + 失败回滚。';
  ...
  readonly 'profiles.save.done': '已保存档案「{name}」';
  ...
  readonly 'profiles.delete.done': '已删除档案「{name}」';
  ...
  readonly 'profiles.import.done': '已导入档案「{name}」';
  ...
  readonly 'import.compatibility.sections': '{count} 个分区';
  readonly 'import.compatibility.plugins': '已装 {installed} · 待装 {toInstall}';
  readonly 'import.compatibility.paths': '{count} 项路径处理';
  readonly 'import.compatibility.secrets': '{count} 项密钥补录';
  readonly 'import.compatibility.score.excellent': '优秀';
  readonly 'import.compatibility.score.good': '良好';
  readonly 'import.compatibility.score.partial': '部分兼容';
  readonly 'import.compatibility.score.unsupported': '不支持';
  readonly 'import.compatibility.sectionsTitle': '备份包含的分区';
  readonly 'sync.status.lastSync': '上次同步';
  readonly 'sync.status.sections': '可同步分区';
  readonly 'nav.overview': '总览';
  readonly 'nav.backups': '备份';
  readonly 'nav.export': '导出';
  readonly 'nav.import': '导入';
  readonly 'nav.sync': '同步';
  readonly 'nav.market': '市场';
  readonly 'nav.profiles': '档案';
  readonly 'nav.activity': '活动记录';
  readonly 'nav.recovery': '灾备';
  readonly 'lifecycle.autoOn': '自动快照已开启';
  readonly 'lifecycle.autoOff': '自动快照未开启';
  readonly 'lifecycle.lastAuto': '最近自动快照';
  readonly 'lifecycle.never': '从未';
  readonly 'lifecycle.undo': '撤销';
  readonly 'lifecycle.redo': '重做';
  readonly 'lifecycle.undoTitle': '撤销上一步配置变更（回退到内容不同的最近快照）';
  readonly 'lifecycle.redoTitle': '重做上一次撤销（仅在撤销后没有新变更时可用）';
  readonly 'lifecycle.undoConfirm': '撤销会回滚配置到上一个不同状态。撤销前会自动保存当前状态，可随时重做。是否继续？';
  readonly 'lifecycle.undoOk': '已撤销到 {id}';
  readonly 'lifecycle.redoOk': '已重做到 {id}';
  readonly 'lifecycle.undoNone': '没有可撤销的变化';
  readonly 'lifecycle.redoNone': '没有可重做的撤销';
  readonly 'lifecycle.snapshotNow': '立即快照';
  readonly 'lifecycle.snapshotReason': '快照原因（可选）';
  readonly 'lifecycle.snapshotCreated': '已创建快照 {id}';
  readonly 'lifecycle.snapshots': '配置状态快照';
  readonly 'lifecycle.empty': '还没有配置状态快照。开启自动快照后，配置变更会自动存档。';
  readonly 'lifecycle.kind.auto': '自动';
  readonly 'lifecycle.kind.manual': '手动';
  readonly 'lifecycle.kind.undo': '撤销';
  readonly 'lifecycle.kind.baseline': '基线';
  readonly 'lifecycle.kind.pre-restore': '撤销前存档';
  readonly 'lifecycle.sections': '{count} 个分区';
  readonly 'lifecycle.delete': '删除';
  readonly 'lifecycle.deleteConfirm': '删除快照 {id}？该快照将无法再用于回退。';
  readonly 'lifecycle.crash.title': '上次启动未正常完成';
  readonly 'lifecycle.crash.lastGood': '最后正常状态：{time}';
  readonly 'lifecycle.crash.restoreLastGood': '回退到最后正常状态';
  readonly 'lifecycle.crash.reason.session-corrupt': '会话文件损坏';
  readonly 'lifecycle.crash.reason.bundle-check': '插件 bundle 无法解析';
  readonly 'lifecycle.crash.reason.patch-tree': '插件加载失败（挂载树冲突）';
  readonly 'lifecycle.crash.reason.unknown': '原因未知';
  readonly 'lifecycle.crash.advice.restore-last-good': '建议回退到最后正常状态。';
  readonly 'lifecycle.crash.advice.repair-session': '建议先用会话扫描修复损坏的会话文件。';
  readonly 'lifecycle.crash.advice.check-bundles': '建议进入救援模式，或检查 package.json 的 dsh.profile.bundles。';
  readonly 'lifecycle.crash.advice.check-patch-tree': '建议进入救援模式禁用插件后逐个排查。';
  readonly 'lifecycle.rescue.title': '救援模式';
  readonly 'lifecycle.rescue.desc': 'DSH 因插件起不来时，把插件挂载收窄到「DSH 核心 + 本插件」：改写两层 cordis.patch.yml，并把 dsh.profile.bundles 里的其它用户插件临时移出。不卸载任何包，退出时从备份完整还原。';
  readonly 'lifecycle.rescue.active': '救援模式已开启（{time}）—— 请重启 DSH 使其生效。';
  readonly 'lifecycle.rescue.enter': '进入救援模式';
  readonly 'lifecycle.rescue.exit': '退出救援模式';
  readonly 'lifecycle.rescue.restartHint': '操作后需要重启 DSH 才能生效。';
  readonly 'lifecycle.rescue.confirm': '进入救援模式会改写 profile 的 cordis.patch.yml、置空 home 的 cordis.patch.yml，并把 dsh.profile.bundles 收窄为 DSH 核心与本插件（其它用户插件本次启动不挂载；包本身不删除）。退出时会从备份逐字节还原。请重启 DSH 使其生效。是否继续？';
  readonly 'lifecycle.rescue.exitConfirm': '退出救援模式将从备份还原 cordis.patch.yml 与 package.json，恢复之前的插件集。是否需要重启 DSH。是否继续？';
  readonly 'lifecycle.rescue.on': '已进入救援模式';
  readonly 'lifecycle.rescue.off': '已退出救援模式';
  readonly 'lifecycle.rescue.inactive': '救援模式未开启';
  readonly 'lifecycle.rescue.stale': '救援标记已失效（换机或重建 home）';
  readonly 'lifecycle.guided.title': '需要更完整的恢复流程？';
  readonly 'lifecycle.guided.desc': '本页是一键回退（撤销/重做）。若 DSH 因中断的事务或崩溃而无法正常恢复，请用「备份」页的恢复向导逐步确认后执行 —— 它会先预览影响、再校验结果。';
  readonly 'lifecycle.guided.action': '打开恢复向导';
  readonly 'nav.about': '关于';
  readonly 'nav.refresh': '刷新';
  readonly 'shell.status.idle': '就绪';
  readonly 'shell.status.running': '{count} 个任务进行中';
  readonly 'shell.status.recovery': '恢复待处理';
  readonly 'shell.version': '插件 {plugin} · DSH {dsh}';
  readonly 'shell.drawer.title': '活动与关于';
  readonly 'overview.result.success': '成功';
  readonly 'overview.result.failed': '失败';
  readonly 'overview.result.skipped': '跳过';
  readonly 'overview.activity.viewAll': '查看全部';
  readonly 'overview.activity.copy': '复制';
  readonly 'overview.activity.redacted': '（文件名已脱敏）';
  readonly 'overview.location.title': '备份位置';
  readonly 'overview.location.dir': '备份目录';
  readonly 'overview.location.totalSize': '备份总体积';
  readonly 'overview.location.retention': '快照保留';
  readonly 'overview.location.retentionValue': '{used} / {limit}';
  readonly 'overview.location.schedule': '定时备份';
  readonly 'overview.location.scheduleOn': '{interval} · 上次 {time}';
  readonly 'overview.location.scheduleOff': '未开启';
  readonly 'overview.location.nextRun': '下次备份';
  readonly 'overview.location.lastRun': '上次备份';
  readonly 'overview.sections.title': '分区构成';
  readonly 'overview.sections.hint': '按当前配置估算（只读预览，零写入）';
  readonly 'overview.sections.total': '合计';
  readonly 'overview.sections.entries': '{count} 项';
  readonly 'overview.quick.exportTitle': '生成可迁移的 ZIP 备份文件并下载';
  readonly 'overview.quick.importTitle': '从备份文件恢复或迁移到本机';
  readonly 'overview.quick.syncTitle': '跨设备保持配置一致（Git / WebDAV）';
  readonly 'overview.quick.backupTitle': '全量快照，随时可回滚';
  readonly 'overview.nav.activity': '活动';
  readonly 'overview.nav.about': '关于';
};
...
declare const zh$2: {
  ...
  readonly 'mode.advancedHint': '手动勾选要推送的分区（等同导出备份的自定义导出）；可移植分区恒可同步，工作区 / 会话属「选择性同步」，需显式勾选。';
  readonly 'mode.sectionsTitle': '同步分区';
  readonly 'mode.sectionsHint': '可移植分区恒可同步。「选择性同步」分区（Workspaces、Sessions）默认不参与 —— 勾选后才会随 WebDAV / Git 通道传输；其余设备 / 平台相关分区（插件文件、凭据状态、MCP）永不参与同步。';
  ...
  readonly 'mode.sectionOptIn': '选择性同步';
  readonly 'mode.optInHint': '「选择性同步」分区默认关闭：工作区记录含绝对路径，会话含完整对话内容（可能含敏感信息）—— 勾选后才会写入远端快照。';
  readonly 'mode.optInWarn': '已勾选「{names}」：其中的绝对路径与会话内容会写入远端快照，请确认远端为可信的私有仓库；换机后路径通常需要重新映射。';
...
declare const zh$1: {
  ...
  readonly 'config.refreshed': '已拉取最新市场目录';
  readonly 'list.empty': '内置市场尚未加载。请先「拉取最新」。';
  readonly 'list.browse': '浏览';
  readonly 'list.browsed': '已重新浏览市场';
  ...
  readonly 'import.done': '导入完成：{count} 项写入';
  readonly 'import.failed': '导入失败（{count} 项失败）';
  readonly 'import.needsRestart': '部分改动需重启 DSH 后生效';
  readonly 'detail.failed': '下载失败，请关闭弹窗后重试（详情见右下角提示）';
  ...
  readonly 'myconfigs.install.failed': '下载失败，请关闭弹窗后重试（详情见右下角提示）';
...
declare const zh: {
  ...
  readonly 'recovery.lock.title': '残留配置锁';
  readonly 'recovery.lock.detailStale': '检测到上次异常退出的配置锁（其持有进程已不存在）。该锁不会自动清除：重试或重启 DSH 都不会恢复，必须在此显式回收，回收后所有操作立即恢复。';
  readonly 'recovery.lock.detailUnknown': '配置锁状态无法可靠判定（可能是崩溃残留）。回收前会再次确认其持有进程确已不存在；无法确认时不会删除任何内容。';
  readonly 'recovery.lock.action': '回收残留锁';
  readonly 'recovery.lock.busy': '正在回收…';
  readonly 'recovery.lock.done': '已回收残留配置锁，操作已恢复';
  readonly 'recovery.lock.refused': '未判定为残留锁，已拒绝回收（未做任何改动）';
  readonly 'recovery.lock.confirmTitle': '回收残留配置锁';
  readonly 'recovery.lock.confirmMessage': '将再次确认该锁的持有进程确已不存在，然后原子移除这把锁。若无法确认（例如锁仍被活跃进程持有），操作会被拒绝且不做任何改动。';
};
/** 字典键联合（注册处 compile-time 校验）。 */
