...
| 🔄 | **远程同步** | 通过 **Git 私有仓库或 WebDAV** 推送 / 拉取可移植配置（密钥永不参与同步）；**工作区 / 会话可选择性同步**（默认关闭，勾选即生效） |
...
除了「可移植分区」，「远程同步 → **高级（自定义导出）** → **同步分区**」列表里还多出两块
**默认关闭、可自由勾选**的用户数据 —— 勾选后随 **WebDAV / Git** 通道一起传输：

| 分区 | 同步的内容 | 默认 |
|:---:|---|:---:|
| **Workspaces（工作区）** | 工作区 **id / 标题 / 绝对路径 / 会话 id 列表**（`storages/workspace.json` 里的工作区记录） | 🔒 关闭 |
| **Sessions（会话）** | 历史会话**文件内容本身**（`~/.dsh/sessions/`，文件级复制） | 🔒 关闭 |

- **默认绝不外泄**：未勾选时这两块分区**不会进入同步快照** —— 默认模式（快速导出 / 默认自动同步）同样不含，工作区绝对路径与会话内容不会被悄悄推上远端。
- **勾选即生效**：Git 与 WebDAV 两条通道各自独立记忆勾选；自动同步后台调度复用同一份分区选择，勾一次就持续生效。
- **工作区只搬「记录」，不搬项目文件**：同步的是工作区注册信息（路径 + 标题 + 会话 id）；工作区**目录里的项目文件**仍需另行搬运（Git / 文件同步工具）。
- **换机需路径映射**：工作区路径是绝对路径，导入时会提示重新映射（支持批量前缀替换）。
- **⚠️ 安全提醒**：会话内容含完整对话（可能有密钥、隐私信息），工作区路径会暴露本机目录结构。请只使用**可信的私有**仓库 / WebDAV；勾选后界面会常驻显示风险横幅。
...
4. **工作区与会话默认不迁移（可选择同步）**：两块分区属「选择性同步」，需在「远程同步 → 高级 → 同步分区」里**显式勾选**才会随 WebDAV / Git 通道传输（文件级复制）；默认导出与默认自动同步恒不含
